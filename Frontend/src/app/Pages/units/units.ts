...
import { SearchService } from '../../services/searchService'; // Using standard naming convention
...
export class Units implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.searchService.searchCriteria$.pipe(
  ...
  extractFilterOptions(unitsToProcess: Unit[]): void {
    ...
    unitsToProcess.forEach(unit => {
      ...
      if (unit.UnitType) { // Assuming UnitType is PascalCase as per Unit.cs DTO
        typeMap[unit.UnitType] = (typeMap[unit.UnitType] || 0) + 1;
      }
    });

    this.villageOptions = Object.keys(villageMap).map(name => ({ name, count: villageMap[name] }));
    this.unitTypeOptions = Object.keys(typeMap).map(name => ({ name, count: typeMap[name] }));
  }
    ...
    if (this.selectedBedrooms) {
      ...
      if (!isNaN(bedroomsNum)) {
        if (this.selectedBedrooms.endsWith('+')) {
          tempUnits = tempUnits.filter(unit =>
            (unit.bedrooms ?? 0) >= bedroomsNum
          );
        } else {
          tempUnits = tempUnits.filter(unit =>
            unit.bedrooms === bedroomsNum
          );
        }
      }
      console.log('After bedrooms filter:', tempUnits.length, 'with criteria:', this.selectedBedrooms);
      console.log('Matching units:', tempUnits.map(u => ({id: u.id, bedrooms: u.bedrooms})));
    }
    ...
    if (this.selectedBathrooms) {
      ...
      if (!isNaN(bathroomsNum)) {
        if (this.selectedBathrooms.endsWith('+')) {
          tempUnits = tempUnits.filter(unit =>
            (unit.bathrooms ?? 0) >= bathroomsNum
          );
        } else {
          tempUnits = tempUnits.filter(unit =>
            unit.bathrooms === bathroomsNum
          );
        }
      }
