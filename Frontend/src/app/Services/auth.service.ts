...
export class AuthService {
  ...
  login(credentials: any): Observable<any> {
    return this.http.post(`${this.baseUrl}/Login`, credentials).pipe(
      tap((response: any) => {
        if (this.isBrowser && response && response.accessToken) { // Guard localStorage access
          localStorage.setItem('accessToken', response.accessToken);
          console.log('Access Token saved:', response.accessToken);

          if (response.userId) { // Guard localStorage access
            ...
            if (extractedUserId) { // Guard localStorage access
          ...
          console.warn('Skipping localStorage operations during SSR for login.');
        } else {
  ...
  isTokenExpired(token: string): boolean {
    if (!token || !this.isBrowser) { // Guard localStorage access and check token
      return true;
    }
