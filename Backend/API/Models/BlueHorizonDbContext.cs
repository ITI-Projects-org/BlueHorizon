    ...
    public class BlueHorizonDbContext : IdentityDbContext<ApplicationUser>
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            var rolesToSeed = new List<IdentityRole>
            {
                ...
                new IdentityRole { Id = Guid.NewGuid().ToString(), Name = "Tenant", NormalizedName = "TENANT", ConcurrencyStamp = Guid.NewGuid().ToString() }
            };
            builder.Entity<IdentityRole>().HasData(rolesToSeed);

            // 2. Seed Admins
            var adminsToSeed = new List<Admin>();
            string[] adminNames = {
                "Ahmed Khaled", "Abdullah Mohamad", "Mark Magdy", 
                "Moomen Abdelshaheed", "Muhammad Sabbagh", "Yousra Ehab"
            };

            var adminRoleId = rolesToSeed.First(r => r.Name == "Admin").Id;
            var adminUserRoles = new List<IdentityUserRole<string>>();

            foreach (var adminName in adminNames)
            {
                var adminId = Guid.NewGuid().ToString();
                var userName = adminName.Replace(" ", "").ToLower();
                
                adminsToSeed.Add(new Admin
                {
                    Id = adminId,
                    UserName = userName,
                    NormalizedUserName = userName.ToUpper(),
                    Email = $"{userName}@bluehorizon.com",
                    NormalizedEmail = $"{userName}@bluehorizon.com".ToUpper(),
                    EmailConfirmed = true,
                    PhoneNumber = random.Next(100000000, 999999999),
                    PhoneNumberConfirmed = true,
                    SecurityStamp = Guid.NewGuid().ToString(),
                    ConcurrencyStamp = Guid.NewGuid().ToString(),
                    PasswordHash = "AQAAAAIAAYagAAAAELBhqnG9N/0lcZn7S/QtT4Eqy/yQEEFPvHNpeMWDXQj3+lY+GRNtWY9NZqrV/Sk9LA==", // Hash for "Admin@123"
                    RegistrationDate = DateTime.Now.AddDays(-random.Next(1, 30)),
                    UserType = "Admin"
                });

                // Add admin role assignment
                adminUserRoles.Add(new IdentityUserRole<string>
                {
                    UserId = adminId,
                    RoleId = adminRoleId
                });
            }
            builder.Entity<Admin>().HasData(adminsToSeed);
            ...
            var ownerRoleId = rolesToSeed.First(r => r.Name == "Owner").Id;
            var ownerUserRoles = new List<IdentityUserRole<string>>();
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                ownerUserRoles.Add(new IdentityUserRole<string>
                {
                    UserId = ownerId,
                    RoleId = ownerRoleId
                });
            ...
            // Add all user roles
            builder.Entity<IdentityUserRole<string>>().HasData(adminUserRoles.Concat(ownerUserRoles));

            // 4. Seed Amenities - ONLY using values from your defined AmenityName enum
